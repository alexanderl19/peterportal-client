...
const Schedule: FC<ScheduleProps> = (props) => {
  ...
  useEffect(() => {
    ...
    axios
      .get<string>("https://peterportal.org/schedule/api/currentQuarter")
      .then((res) => {
        setQuarter(res.data);
  ...
  const fetchScheduleDataFromAPI = async (currentQuarter: string) => {
    ...
    if (props.courseID) {
      ...
      url = `https://peterportal.org/schedule/api/${currentQuarter}/${department}/${number}`;
    } else if (props.professorID) {
      url = `https://peterportal.org/schedule/api/${currentQuarter}/${props.professorID}`;
    }
    ...
    if (section.status == "OPEN") {
      return (
        ...
        <Button
          variant="light"
          size="lg"
          className="btn-status-button-open btn-status"
          disabled={true}
        >
          {" "}
          OPEN{" "}
      ...
      return (
        ...
        <Button
          variant="light"
          size="lg"
          className="btn-status-button-waitl btn-status"
          disabled={true}
        >
          {" "}
          WAITLIST{" "}
      ...
      return (
        ...
        <Button
          variant="light"
          size="lg"
          className="btn-status-button-full btn-status"
          disabled={true}
        >
          {" "}
          FULL{" "}
  ...
  const renderProgressBar = (section: Section) => {
    ...
    let percentage =
      (Number(section.numCurrentlyEnrolled.totalEnrolled) * 100) /
      Number(section.maxCapacity);
    if (section.status == "OPEN") {
      return (
      ...
      <tr key={index}>
        ...
        <td className="enrollment-col">
          <span className="enrollment-info-text">
            {Number(section.numCurrentlyEnrolled.totalEnrolled)} /{" "}
            {Number(section.maxCapacity)}
          </span>
